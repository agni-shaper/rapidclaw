...
def parse_sprint(date_str: str) -> list[str]:
    ...
    tpls = []
    for line in path.read_text().splitlines():
        if re.match(rf"^## {re.escape(date_str)}\b", line.strip()):
            in_today = True
            continue
        if line.startswith("##") and in_today:
            break
        if in_today:
            m = re.match(r"^- (TPL-[A-Z0-9-]+)", line.strip())
            if m:
...
def infer_template_from_bullet(bullet: str) -> Optional[str]:
    ...
    # Blog task (synthetic)
    if "publish a blog for rapidnative" in b: return "BLOG-TASK"
    return None
...
def distribute_engagement_findings(all_tasks_by_crew: dict, recon: dict):
    """Round-robin single-target findings across crew. Multi-target gets the full list.
    Recon cache uses short platform keys (HN, Reddit, Quora, LinkedIn, Twitter, Facebook);
    TEMPLATE_DEFS uses display names (Hackernews, Reddit, Quora, LinkedIn, Twitter, Facebook).
    Map between them below.
    """
    ...
    for plat, block in recon.get("findings", {}).items():
        if block.get("status") == "ok":
            findings_by_platform[plat] = block.get("findings", [])

    # Single-target distribution: collect all single-target tasks per platform across all crew + tasks
    single_targets: dict = {}  # platform -> [(sid, task)]
    for sid, tasks in all_tasks_by_crew.items():
        for t in tasks:
            tpl = TEMPLATE_DEFS.get(t["template_id"], {})
            if tpl.get("shape") == "B":  # community-posting = single-target
                plat = display_to_recon_key.get(tpl["platform"], tpl["platform"])
                single_targets.setdefault(plat, []).append(t)
    for plat, tasks in single_targets.items():
        findings = findings_by_platform.get(plat, [])
        if not findings:
            continue
        for i, t in enumerate(tasks):
            t["recon_finding"] = findings[i % len(findings)]

    # Multi-target: every multi-target task on a platform gets the full findings list
    for sid, tasks in all_tasks_by_crew.items():
        for t in tasks:
            tpl = TEMPLATE_DEFS.get(t["template_id"], {})
            if tpl.get("shape") == "C":  # community-engagement = multi-target
                plat = display_to_recon_key.get(tpl["platform"], tpl["platform"])
                findings = findings_by_platform.get(plat, [])
                if findings:
...
def attach_personal_drafts(all_tasks_by_crew: dict, recon: dict):
    """Per crew × per personal template, attach the matching original_posts draft."""
    ...
    for tpl_id, plat_key in template_to_platform_key.items():
        block = op.get(plat_key, {})
        if block.get("status") != "ok":
            continue
        ...
        for sid, tasks in all_tasks_by_crew.items():
            for t in tasks:
                if t["template_id"] == tpl_id:
                    if d := drafts_by_user.get(sid):
                        t["personal_draft"] = d


def attach_article_drafts(all_tasks_by_crew: dict, recon: dict):
    """Per crew × per article template, attach matching article_drafts entry."""
...
def write_snapshot(sentinel: dict, today: str, wlabel: str, on_leave: list):
    ...
    out.append(f"_Generated by gen-marketing-morning.py at {datetime.datetime.now().strftime('%H:%M IST')}. "
               f"Edit `.claude/skills/growth-marketing/references/sprint.md` or `.claude/skills/growth-marketing/references/rotation.md` and rerun to regenerate._")
    out.append("")
...
def main():
    ...
    # Parse inputs
    today_templates = parse_sprint(today)
    if not today_templates:
        msg = f"🟠 *marketing-morning skipped* — sprint.md has no section for {today}. Edit .claude/skills/growth-marketing/references/sprint.md and rerun: accountability/routines/gen-marketing-morning.py"
        if not args.dry_run:
            slack_post(msg)
    ...
    # Build per-crew task lists
    all_tasks_by_crew: dict = {}
    for m in working:
        sid = m["slack_id"]
        new_tasks = build_task_list(m, today_templates, accounts, offsets, W, wlabel)
    ...
    print(f"templates today: {today_templates}")
    print(f"recon: {bool(recon)}, blog: {bool(blog_cache)}")
    print()
    total_posted = 0
    total_enriched = 0
    total_failed = 0
