...
def write_snapshot(sentinel: dict, today: str, wlabel: str, on_leave: list):
    ...
    out.append(f"_Generated by gen-marketing-morning.py at {datetime.datetime.now().strftime('%H:%M IST')}. "
               f"Edit `marketing/sprint.md` or `marketing/rotation.md` and rerun to regenerate._")
    out.append("")
...
def main():
    ...
    if not today_templates:
        msg = f"🟠 *marketing-morning skipped* — sprint.md has no section for {today}. Edit marketing/sprint.md and rerun: accountability/routines/gen-marketing-morning.py"
        if not args.dry_run:
            slack_post(msg)
