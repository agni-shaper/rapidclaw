...
| `sprint-rollover` | 05:45 Mon | (file only — no Slack) | ✅ deterministic Python; auto-appends this week's Mon→Fri sections to `sprint.md`. Runs **before `marketing-recon` at 06:00** so recon reads a populated sprint on Monday morning (was 06:30 until 2026-07-20 — moved earlier because recon reads sprint.md and ships empty when today's section is missing). |
