...
| `growth-marketing/social-engagement` | **LIVE — sub-skill (since 2026-07-01)** — the actual daily distribution spine (marketing-recon/morning/evening + gtm-weekly-pick + biweekly-shoutouts all depend on this sub-skill's references dir). Not auto-discovered — the umbrella loads it. | Daily social platform cycle. Holds `references/{accounts,rotation,config,sprint,task-templates}.md` + `strategies/{rapidnative,applighter,letsdeployit}.md`. | (was `growth-marketing/references/*` pre-2026-07-01) | `#marketing-automation` |
