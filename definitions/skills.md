...
| `bug-tracking` | **LIVE — sqlite cutover 2026-07-02** — referenced by task-management when bug-shaped signals are detected; no standalone routine. Bugs are sqlite `tasks` rows with `category='bug'`, written via `tasks.sh`. Replaces `sites/tasks/intake/bugs.md` (archival). | 12:15 scan feeds sqlite `tasks` rows with `category='bug'`. | (currently part of tasks-cleanup) | `#rapidnative-coach`, `#tasks` |
