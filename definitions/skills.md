...
|---|---|---|---|---|
| `growth-marketing` | **LIVE (Phase 2 + plist cutover 2026-06-30)** — loaded by marketing-recon.v2, marketing-morning.v2, marketing-evening.v2, gtm-weekly-pick.v2, biweekly-shoutouts.v2 (all in cron). 4 reference files: 3 brand strategies + accounts + rotation + config pointer. | Distribution / growth ops across all 3 products. Per-brand strategies + per-account references. | `marketing/` directory + `marketing-morning`/`evening`/`recon` routines body | `#marketing-automation`, `#marketing` |
| `task-management` | **LIVE (Phase 2 + plist cutover 2026-06-30)** — loaded by tasks-cleanup.v2 (in cron) + gtm-weekly-pick.v2 (post-approval route flow). Tier A feeder + Tier B DB (sites/tasks/) split. | Feed `sites/tasks/` from #standup, #eod, #user-testing, git logs. 12:15 IST cleanup proposal. | `tasks-cleanup` routine body | `#tasks`, `#rapidnative-coach` |
| `leave` | **LIVE (Phase 2 + Phase 3 sqlite 2026-06-29)** — used by every team-facing v2 routine via is_on_leave / sqlite_is_on_leave helpers. Markdown helpers in _lib.sh stay as the production read path; sqlite parity verified. | Sqlite leave_entries + holidays CRUD. `is_on_leave`, `is_working_day`. | `accountability/leave.md` + `accountability/holidays.md` (kept as projections) | any team-facing |
| `eod-nudges` | **LIVE (Phase 2 + plist cutover 2026-06-29)** — loaded by eod-streak-check.v2 (in cron). Threshold raised 2→3 wd 2026-06-30. Composes with leave skill. | Detect missing EOD posts, ping with leave-awareness. | `eod-streak-check` body | `#eod-updates` |
| `user-testing` | **LIVE (Phase 2 + plist cutover 2026-06-30)** — loaded by user-testing-capture.v2 (in cron). Read-only against #user-testing; write-only against issues-log.md (post-approval). Bug-vs-UX split rule composes with bug-tracking. | Daily diff of `#user-testing`, projection to `accountability/user-testing/issues-log.md`. | `user-testing-capture` body | `#user-testing` |
| `weekly-wrap` | **SCAFFOLDED — NOT IN CRON (Phase 2)** — invoked on-demand via cross-channel routine `accountability/routines/cross-channel/weekly-wrap.md`. Defers to RN site's content-studio-generate-weekly-wrap skill. | Cross-channel composer for marketing-team Friday recap. | `accountability/routines/cross-channel/weekly-wrap.md` + `sites/rapidnative-website/.claude/skills/content-studio-generate-weekly-wrap` | source thread → `#marketing` |
...
| `bug-tracking` | **LIVE-VIA-COMPOSITION (Phase 2)** — referenced by task-management when bug-shaped signals are detected; no standalone routine yet. | 12:15 scan feeds `sites/tasks/intake/bugs.md`. | (currently part of tasks-cleanup) | `#rapidnative-coach`, `#tasks` |
