...
| `scheduler` | **SCAFFOLDED — DESIGN ONLY (Phase 2, 2026-06-29)** — `.claude/skills/scheduler/SKILL.md`. Sketches Phase 3 sqlite schema (reminders + routine_runs) + proposed `_lib.sh` helpers (`add_reminder`, `list_reminders`, `log_routine_{start,end}`, `last_run`). Inert until Phase 3. | Sqlite reminders + routine_runs CRUD. plists still fire cron; state moves to DB. | `accountability/reminders/*.md` + `accountability/state/*.json` | (utility, not channel-bound) |
